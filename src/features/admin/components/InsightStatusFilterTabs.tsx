...
/**
 * Module 9E — status filtering for `/admin/insights` (spec §7), same
 * shape as `TeamMemberStatusFilterTabs.tsx`/`ServiceStatusFilterTabs.tsx`/
 * `ProjectStatusFilterTabs.tsx`: a plain link list so the filter is a
 * real `?status=...` navigation the server-rendered list page reads
 * to build its database query, not client-side state filtering
 * records that were already fetched.
 */
export function InsightStatusFilterTabs({ active }: { active: ContentStatus | "all" }) {
  ...
  return (
    <nav aria-label="Filter insights by status" className="flex flex-wrap gap-2">
      {tabs.map((tab) => {
        const isActive = tab.value === active;
        const href = tab.value === "all" ? "/admin/insights" : `/admin/insights?status=${tab.value}`;
        return (
          <Link
            key={tab.value}
            href={href}
